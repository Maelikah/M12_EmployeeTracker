...
function getRolesList() {
    ...
    return new Promise((resolve, reject) => {
        const getRolesListQuery = `SELECT role.id, role.title, role.department_id FROM role`;
        dbConnection.query(getRolesListQuery, (error, results) => {
                if (error) {
                    ...
                    const roles = results.map((result) => {
                        return { role_id: result.id, title: result.title, department: result.department };
                    });
                    resolve(roles);
                    }
        });
